...
# Fork (#83714): shrunk tool-call args are replayed as the model's OWN past output, so the marker
# must look nothing like ``...[truncated]``. Marker text lives in tools.truncation_markers so the
# write/patch guard can refuse the same token if a model ever copies it (#83752 / #83843).
from tools.truncation_markers import (  # noqa: E402
    COMPRESSION_MARKER_TEMPLATE as _COMPRESSION_MARKER_TEMPLATE,  # noqa: F401
...
def _truncate_tool_call_args_json(args: str, head_chars: int = 200) -> str:
    ...
    The shrunk value stays a plain string (never a nested object) so the JSON *shape* is unchanged;
    only the marker text differs from the upstream ``...[truncated]`` (see ``_COMPRESSION_MARKER_TEMPLATE``)."""
    try:
    ...
    def _shrink(obj: Any) -> Any:
        if isinstance(obj, str):
            if len(obj) > head_chars:
                return obj[:head_chars] + _format_compression_marker(omitted=len(obj) - head_chars, total=len(obj))
            return obj
        if isinstance(obj, dict):
