...
class ReasoningParamsMixin:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if base_url_host_matches(url, "nousresearch.com") or base_url_host_matches(url, "ai-gateway.vercel.sh"):
            return True
        # Fork: ClinePass forwards to OpenRouter, which accepts the reasoning object for the whole
        # cline-pass/ catalog (all reasoning-capable); verified live 2026-08-22 with both
        # reasoning_effort and extra_body.reasoning shapes.
        if base_url_host_matches(url, "cline.bot"):
            return True
