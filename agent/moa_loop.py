...
# ── Advisor cooldown (fork: fail-fast on exhausted advisor providers) ─────────────────
# A reference advisor that fails on a rate-limit / upstream / timeout error is skipped for
# ``reference_cooldown_seconds`` so an exhausted free tier doesn't make every subsequent turn
# wait on a dead endpoint (per-advisor timeout + fallback-chain walk, repeated per turn).
# Keyed by normalized (provider, model); in-memory only — resets on gateway restart.
...
def _reference_is_cooling_down(slot: dict[str, Any]) -> bool:
    ...
    return key is not None and _REFERENCE_COOLDOWN_UNTIL.get(key, 0.0) > time.monotonic()
...
def _run_reference(
    ...
    cache_disabled: bool | None = None, cache_ttl: str | None = None, reference_cooldown_seconds: float = 0.0,
) -> tuple[str, str, Any]:
    ...
    try:
        ...
        # Fork fail-fast: cool this advisor down so the next turns skip it.
    ...
    try:
        for idx, slot in enumerate(reference_models):
            ...
            if _reference_is_cooling_down(slot):
                # Fork: advisor still cooling down from a prior failure — skip the attempt entirely.
                results[idx] = _placeholder_output(slot, _REFERENCE_COOLDOWN_NOTE)
                continue
            futures[executor.submit(
                ...
                cache_disabled=cache_disabled, cache_ttl=cache_ttl, reference_cooldown_seconds=reference_cooldown_seconds,
            )] = idx
...
class MoAChatCompletions:
    ...
    def _run_fanout(
        ...
        # Fork: seconds a failed advisor is skipped before the fan-out retries it (0 = disabled);
        # normalized by moa_config (600s default).
        try:
            reference_cooldown_seconds = float(preset.get("reference_cooldown_seconds") or 0.0)
        except (TypeError, ValueError):
