...
@pytest.mark.asyncio
async def test_status_command_uses_coherent_persisted_model_route(tmp_path):
    """Persisted status must not combine a model and provider from different calls.

    The latest coherent main-loop tuple wins (recency, so a provider switch is
    reflected immediately); the #87227 invariant is that the legacy sessions
    row's franken-tuple (model from one call, provider from another) never
    appears.
    """
    session_entry = SessionEntry(
        session_key=build_session_key(_make_source()),
