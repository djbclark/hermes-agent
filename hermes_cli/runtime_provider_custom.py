...
def _apply_custom_provider_extras(custom_provider: Dict[str, Any], target_model: Optional[str], result: Dict[str, Any]) -> None:
    ...
    # Fork: OpenCode Zen's OpenAI-compatible endpoint requires x-api-key rather than
    # Authorization: Bearer. Named custom aliases (e.g. the local opencode-zen-free free-model
    # lane) otherwise resolve fine but every request gets HTTP 401. Scoped to Zen, not Go
    # (OpenCode Go accepts the normal Bearer header). SECURITY: never log the value.
    _api_key = result.get("api_key")
    _base_url = str(result.get("base_url") or custom_provider.get("base_url") or "")
    if (
        isinstance(_api_key, str) and _api_key
        and _rp().base_url_host_matches(_base_url, "opencode.ai")
        and "/zen/v1" in _base_url.lower() and "/zen/go/" not in _base_url.lower()
    ):
        extra_headers.setdefault("x-api-key", _api_key)
    if extra_headers:
        result["extra_headers"] = extra_headers
