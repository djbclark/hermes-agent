...
def _validate_discovery_disabled(req: _Request) -> Optional[dict[str, Any]]:
    """Fork: honour ``discover_models: false`` on a configured provider. Some real APIs have no
    /models endpoint at all (ClinePass at api.cline.bot 404s), so every live probe would fail and
    fall through to the generic "could not reach the API" warning even though the model works.
    Validate directly against the configured ``models:`` list instead — same semantics as the
    picker paths. Returns None (fall through) for providers that keep discovery on."""
    try:
        from hermes_cli.config import get_compatible_custom_providers

        cfg = next((p for p in get_compatible_custom_providers()
                    if str(p.get("provider_key", "")).strip().lower() == req.normalized), None)
    except Exception:
        cfg = None
    if cfg is None or cfg.get("discover_models") is not False:
        return None
    configured = [str(m) for m in (cfg.get("models") or {}) if str(m).strip()]
    if not configured:
        # No explicit list to check against and the probe is doomed: accept quietly.
        return _verdict(True, True, False, None)
    lower = {m.lower(): m for m in configured}
    if req.lookup.lower() in lower:
        return _accept()
    auto = get_close_matches(req.lookup.lower(), list(lower), n=1, cutoff=0.9)
    if auto:
        corrected = lower[auto[0]]
        return {**_accept(), "corrected_model": corrected,
                "message": f"Auto-corrected `{req.requested}` → `{corrected}`"}
    suggestions = get_close_matches(req.lookup.lower(), list(lower), n=3, cutoff=0.5)
    suggestion_text = ("\n  Similar models: " + ", ".join(f"`{lower[s]}`" for s in suggestions)) if suggestions else ""
    return _soft_accept(f"Note: `{req.requested}` was not found in the configured `models:` list "
                        f"for this provider.{suggestion_text}")
...
_LADDER: tuple[tuple[Callable[[_Request], bool], Callable[[_Request], Optional[dict[str, Any]]]], ...] = (
    ...
    (lambda req: True, _validate_discovery_disabled),
