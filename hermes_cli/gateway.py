...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``external_supervisor=True`` also drops ``--replace``: launchd must not
    run gateway takeover logic on every respawn (see
    docs/rca-launchd-macos-crash-loop-protection-2026-08-08.md).
    """
    inner = _gateway_run_command()
    if external_supervisor:
        inner = [arg for arg in inner if arg != "--replace"]
