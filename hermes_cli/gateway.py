...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    if is_macos():
        # Fork: an operator-managed LaunchAgent (see _find_foreign_launchd_gateway) is
        # service-managed too and must not be swept as a manual process.
        try:
...
# ── Fork: operator-managed (foreign-label) launchd gateway ────────────────────────────────
# Some operators manage the gateway LaunchAgent themselves (Ansible, dotfiles, ...) under their
# own label rather than ``ai.hermes.gateway``. ``get_launchd_plist_path()`` then does not exist
# and the stock stop/start/restart paths fight that job's ``KeepAlive`` (launchd respawns its copy
# within ThrottleInterval; a foreground gateway we start is torn down by ``--replace`` or exits on
# "another gateway instance started during our startup"). These helpers drive the owning label
...
def _find_foreign_launchd_gateway() -> tuple[str, str, int] | None:
    """``(label, domain, pid)`` when a launchd job hermes did NOT install supervises this profile's
    running gateway; None when no such job owns the gateway PID (or off macOS)."""
    if not is_macos():
        return None
    from gateway.status import get_running_pid
    ...
    if pid is None:
        return None
    try:
        result = subprocess.run(["launchctl", "list"], capture_output=True, text=True, encoding="utf-8",
...
def _restart_foreign_launchd_gateway(label: str, domain: str, pid: int) -> None:
    """Drain-then-relaunch a gateway owned by a non-hermes launchd job. Mirrors ``launchd_restart()``'s
    drain step but never bootstraps our own plist: the operator's job stays the single owner. After
    the old PID exits an unconditional ``KeepAlive`` usually respawns the job; only when launchd has
    not done so do we ``kickstart`` it. ``CalledProcessError``/``TimeoutExpired`` propagate."""
    target = f"{domain}/{label}"
    print(f"→ Gateway PID {pid} is supervised by launchd job {label} (restarting via {target})")
    # SIGUSR1 = in-band graceful restart (refuse new turns, drain, exit 75 → KeepAlive respawns);
    # SIGTERM is only the fallback when it cannot be delivered or the budget runs out.
    wait_budget = _get_restart_exit_wait_budget()
    print(f"→ Restarting gateway (PID {pid}) gracefully — waiting for in-flight runs (up to {wait_budget:.0f}s)...")
    ...
    # KeepAlive respawn is not instantaneous (ThrottleInterval); give it a moment before forcing a
    # kickstart so we don't restart the job twice.
...
def _find_foreign_launchd_gateway_plist() -> tuple[str, Path] | None:
    """``(label, plist_path)`` for an operator-managed gateway LaunchAgent: scans the real account
    ``~/Library/LaunchAgents`` for a plist whose ``ProgramArguments`` run ``hermes ... gateway run``
    under a label that is not ours (``ai.hermes.gateway*``). Unlike ``_find_foreign_launchd_gateway``
    this works while the job is booted out (no live PID), which is what ``start`` needs after a
    ``stop``. First match in sorted filename order."""
    if not is_macos():
        return None
    import plistlib
...
def _stop_foreign_launchd_gateway(label: str, domain: str, pid: int) -> None:
    """Boot out an operator-managed gateway LaunchAgent so KeepAlive stays quiet (a plain SIGTERM
    would just be respawned within ThrottleInterval, which is why ``stop`` used to look like a
    no-op). ``hermes gateway start`` re-bootstraps the operator's own plist."""
    target = f"{domain}/{label}"
...
def _start_foreign_launchd_gateway(label: str, plist_path: Path) -> None:
    """Bootstrap + kickstart an operator-managed gateway LaunchAgent. Never regenerates or rewrites
    the plist — the operator's file is the single owner. Idempotent when already running."""
    domain = _probe_launchd_domain_for_label(label)
    target = f"{domain}/{label}"
    loaded, pid = _launchd_print_service_pid(domain, label)
    if loaded and pid:
...
def launchd_install(force: bool = False):
    ...
    # Fork: refuse to install ai.hermes.gateway beside an operator-managed gateway LaunchAgent —
...
def _stop_installed_service(system: bool) -> bool:
    ...
    if kind is None:
        # Fork: operator-managed LaunchAgent (see _find_foreign_launchd_gateway): boot the job
        # out; a PID kill would just be respawned by KeepAlive.
        if is_macos() and (foreign := _find_foreign_launchd_gateway()) is not None:
            try:
...
def _cmd_start(args):
    ...
    backend = _service_backend()
    if backend == "launchd" and not get_launchd_plist_path().exists():
        # Fork: operator-managed LaunchAgent under its own label — load THAT rather than
        # regenerating ai.hermes.gateway.plist next to it (two KeepAlive jobs would fight).
...
def _restart_all(system: bool) -> None:
    ...
    if kind is None and is_macos() and (foreign_plist := _find_foreign_launchd_gateway_plist()) is not None:
        # Fork: operator-managed LaunchAgent (booted out above) — reload it rather than running a
        # foreground gateway against nothing.
        _start_foreign_launchd_gateway(*foreign_plist)
    elif kind is None:
...
def _cmd_restart(args):
    ...
    if kind is None and is_macos():
        # Fork: operator-managed LaunchAgent under a label of their own — restart through it
        # rather than killing the process and racing its KeepAlive with a foreground run.
        foreign = _find_foreign_launchd_gateway()
        if foreign is not None:
            ...
            try:
                ...
                pass
        elif not get_launchd_plist_path().exists() and not find_gateway_pids():
            # Foreign job booted out (after `hermes gateway stop`) and no gateway runs at all:
            # restart == start the operator's job.
            foreign_plist = _find_foreign_launchd_gateway_plist()
            if foreign_plist is not None:
                service_configured = True
                try:
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        if pids:
            ...
            if foreign is not None:
                # Fork: operator-managed LaunchAgent — say so instead of "manual", and skip the
                # install hint (installing ai.hermes.gateway beside it would fight it).
