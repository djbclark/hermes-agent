...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
                             ...
                             allow_paid_opencode_zen: bool = False) -> Dict[str, Any]:
    ...
    if requested_provider == "opencode-zen" and not allow_paid_opencode_zen:
        _enforce_opencode_zen_free_only(target_model)
    return next(r for r in _ladder_rungs(requested_provider, explicit_api_key, explicit_base_url, target_model) if r)


def _enforce_opencode_zen_free_only(target_model: Optional[str]) -> None:
    """Fork no-spend policy: ``opencode-zen`` may only route FREE models unless
    ``model.allow_paid_opencode_zen: true`` (config) or the caller passed an explicit
    ``--provider`` (``allow_paid_opencode_zen=True``). A model in the VERIFIED keyless catalog
    routes anonymously on the Zen relay (``opencode_zen_free_runtime``) — it cannot spend, so it
    is admitted even when models.dev carries no pricing row for it. Everything else fails closed
    with ``opencode_zen_policy_error``."""
    _full_cfg = _config_mod.load_config()
    _model_cfg = _full_cfg.get("model") if isinstance(_full_cfg, dict) else {}
    _model_cfg = _model_cfg if isinstance(_model_cfg, dict) else {}
    if bool(_model_cfg.get("allow_paid_opencode_zen", False)):
        return
