...
# Fork: seconds a failing reference advisor stays "cooled down" (skipped by the fan-out) before
# it is attempted again. 600s = 10 min. 0 disables the cooldown. In-memory only — resets on
# gateway restart.
...
def _coerce_reference_cooldown(value: Any) -> float:
    """Per-preset advisor cooldown in seconds (0 = disabled). ``None``/blank/bool, a
    non-number, or a negative / non-finite value all fall back to the 600s default so a
    malformed value can never silently disable the safeguard (fork)."""
...
def _normalize_preset(raw: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "reference_cooldown_seconds": _coerce_reference_cooldown(raw.get("reference_cooldown_seconds")),
...
_FLAT_PRESET_KEYS = (
    ...
    "reference_timeout", "degraded_reference_policy", "reference_cooldown_seconds",
    "fanout", "enabled")
