...
def _restart_macos_launchd_gateways(
    ...
    # Fork: operator-managed LaunchAgent under a label of its own (no ai.hermes.gateway plist):
    # restart it through launchd like the fleet, so the manual-process sweep never SIGTERMs a
    # KeepAlive job (it would just be respawned, and the update would look like a no-op).
    try:
        from hermes_cli.gateway import _find_foreign_launchd_gateway, _restart_foreign_launchd_gateway

        foreign = None if get_launchd_plist_path().exists() else _find_foreign_launchd_gateway()
    except Exception:
        foreign = None
