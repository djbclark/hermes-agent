...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def get_latest_session_model_usage(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        actually handled the latest API call. Only coherent main-loop tuples
        qualify (same filters as ``get_dominant_session_model_route``) so an
        auxiliary-task call can never masquerade as the session route.
        """
        self.flush_token_counts()
        with self._read_ctx() as conn:
            row = conn.execute(
