...
def _pq_record_to_pending(record: Dict[str, Any], subsystem: str) -> Dict[str, Any]:
    """Reshape a memory_pending_queue journal record into the pending-record shape
    ``stage_write`` / ``list_pending`` callers expect (``id``, ``subsystem``, ``action``,
    ``summary``, ``origin``, ``created_at``, ``payload``). ``_pq._row_to_record`` already
    parsed the JSON ``payload`` back into a dict."""
    return {
        "id": record.get("id", ""), "subsystem": subsystem, "action": record.get("action", ""),
        "summary": record.get("summary", ""), "origin": record.get("origin", "foreground"),
        "created_at": record.get("created_at", time.time()), "payload": record.get("payload", {}),
    }
...
def stage_write(subsystem: str, payload: Dict[str, Any], *, summary: str, origin: str) -> Dict[str, Any]:
    ...
    MEMORY records live in the SQLite pending-operation journal (fork capacity guard,
    ``tools.memory_pending_queue``); SKILLS keep the legacy per-record JSON files."""
    pid = uuid.uuid4().hex[:8]
    if subsystem == MEMORY:
        try:
            pq_record = _pq.enqueue(
                kind=_pq.KIND_APPROVAL, action=payload.get("action", ""), target=payload.get("target", ""),
                payload=payload, summary=summary, origin=origin,
            )
            return _pq_record_to_pending(pq_record, subsystem)
        except Exception as e:  # pragma: no cover - disk / cap failure path
            logger.error("Failed to stage pending %s write via journal: %s", subsystem, e, exc_info=True)
            return {
