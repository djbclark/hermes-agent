...
def _ensure_mcp_loop():
    ...
    with _core._lock:
        if _origin._mcp_loop is not None and _origin._mcp_loop.is_running():
            return
        # Fork: tolerant tools/list (drops a malformed tool instead of parking the server with zero
        # tools — see tools/mcp_tolerant_list.py). Idempotent; never fatal.
        try:
            from tools.mcp_tolerant_list import install_tolerant_list_tools
            install_tolerant_list_tools()
        except Exception:
