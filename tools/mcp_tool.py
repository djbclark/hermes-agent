...
# ---------------------------------------------------------------------------
# Defensive tools/list parsing for MCP servers with malformed tool schemas.
#
# Some external MCP servers (e.g. Bezalel) publish a tools/list where ONE tool
# has a structurally invalid inputSchema (``{"anyOf": [{"type": "object"},
# {"type": "array"}]}`` with no top-level ``type``). mcp 2.x then raises
# pydantic.ValidationError on the whole ListToolsResult and parks the server
# with zero tools. Always-on: only activates after a strict validation failure
# located under the ``tools`` field. Reviewers (claude-code, agy gemini-3.1-pro-high,
# grok) agreed on lock + ValidationError-typed filter; claude additionally
# required mirroring send_request plumbing (_stamp, timeout, absorb).
...
async def _lenient_list_tools(session, params):
    ...
    raw = await session._dispatcher.send_raw_request(
        "tools/list", data.get("params"), opts
    )
    ttl = raw.get("ttlMs")
    ...
    complete = (
        params is None or getattr(params, "cursor", None) is None
...
def _install_tolerant_list_tools() -> None:
    """Install a tolerant list_tools wrapper (idempotent, thread-safe)."""
        ...
        async def _tolerant_list_tools(self, *args, **kwargs):
            ...
            if not getattr(self, "_hermes_list_tools_lenient", False):
                try:
                    ...
                    strict_exc = exc
                    logger.warning(
                        "tools/list strict-validation failed (%s); dropping malformed tools",
                        type(exc).__name__,
                    )
            try:
                ...
                if strict_exc is not None:
                    logger.warning(
                        "MCP tolerant tools/list fallback failed (%s: %s); "
                        "surfacing original validation error",
...
def _ensure_mcp_loop():
    ...
    with _lock:
        if _mcp_loop is not None and _mcp_loop.is_running():
            return
        # Install the tolerant list_tools monkey-patch (defensive against MCP
        # servers that publish a tool with an invalid inputSchema — see
        # _install_tolerant_list_tools for the rationale). Idempotent.
        try:
            _install_tolerant_list_tools()
        except Exception:
            logger.warning("tolerant list_tools install failed", exc_info=True)
