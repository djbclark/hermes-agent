...
# Fork (#83714 / #68512 / #83843): refuse AI truncation placeholders and Hermes compressor markers
# on the write path. Detection lives in tools.truncation_markers (shared with the context
# compressor) and is count-based when an original baseline is available, so pre-existing
# docs/tests that mention markers are not blocked but adding a NEW occurrence still is.
from tools.truncation_markers import find_new_truncation_placeholder as _find_truncation_placeholder  # noqa: E402


def _extract_v4a_added_content(patch_content: str) -> str:
    """Only the text a V4A patch is ADDING ('+' hunk lines and full Add-File bodies). Falls back
    to the raw patch text if it fails to parse so detection degrades gracefully."""
    try:
    ...
    try:
        ...
        with ExitStack() as _locks:
            ...
            if mode == "replace":
                ...
                # Fork #83714 / #68512 — count-based: only flag markers NEW relative to old_string.
                _placeholder = _find_truncation_placeholder(new_string, old_string)
                if _placeholder:
                    return tool_error(_truncation_placeholder_error("new_string", _placeholder))
                _replace_target = _path_to_resolved.get(path) or path
                result = file_ops.patch_replace(_replace_target, old_string, new_string, replace_all)
            elif mode == "patch":
                if not patch:
                    return tool_error("patch content required")
                # Fork #83714 — scan only the content the model is ADDING ('+' hunk lines and
                # Add-File bodies), never the whole patch text (context lines may legitimately
                # anchor on a pre-existing placeholder-like literal).
                _placeholder = _find_truncation_placeholder(_extract_v4a_added_content(patch))
                if _placeholder:
                    return tool_error(_truncation_placeholder_error("patch content", _placeholder))
