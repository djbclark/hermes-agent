...
# FORK(memory-capacity-guard): rebind MemoryStore to the guarded subclass and re-export the guard API
# (tools/memory_capacity_guard.py). Consumers import these names from here.
from tools.memory_capacity_guard import (  # noqa: E402,F401
    GuardedMemoryStore as MemoryStore, MEMORY_QUEUE_PCT, MEMORY_TARGET_PCT, MEMORY_WARN_PCT, PIN_MARKER,
    consolidate_entries, entries_char_total, is_pinned, target_char_budget)
...
def _validate_single_op(store, action, target, content, old_text) -> Optional[str]:
    ...
    if action in ("replace", "remove") and not old_text:
        return json.dumps({
            ...
            "current_entries": store.entries_for_read(target),  # FORK(memory-capacity-guard): show queued writes too
            "usage": store._usage(target)}, ensure_ascii=False)
