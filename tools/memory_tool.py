...
# Conservative pin convention for the deterministic projection consumer's
# eviction policy (tools/memory_projection.py). The §-delimited entry format
# has no metadata sidecar, so "pinned" is encoded directly in the entry text:
# an entry is pinned if it STARTS with this literal marker (after stripping
# whitespace), case-insensitively. There is no separate unpin bookkeeping --
# removing the marker text (a normal `replace`) unpins it. This is
# deliberately conservative: unmarked entries are NEVER assumed pinned, so
# eviction only ever protects entries a human/agent explicitly tagged.
PIN_MARKER = "[pinned]"


def is_pinned(entry: str) -> bool:
    """True if *entry* carries the projection consumer's pin marker.

    Pinned entries are never chosen for deterministic eviction (see
    ``MemoryStore.apply_with_capacity``) -- the marker is the only priority
    signal the current entry format supports.
    """
