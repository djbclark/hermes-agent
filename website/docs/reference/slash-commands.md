...
| `/clinepass [low\|medium\|high\|xhigh\|max] [--global]` | (Fork) Switch to ClinePass at a capability level: sets the `cline-pass/*` model **and** the reasoning effort in one step. Bare `/clinepass` shows the level table; `--global` persists both to `config.yaml`. |
...
| `/aiuse` | (Fork) Show live AI subscription and API usage across the operator's providers, directly (no AI turn). |
...
| `/clinepass [level] [--global]` | (Fork) Switch to ClinePass at a capability level (`low`/`medium`/`high`/`xhigh`/`max`). Bare `/clinepass` opens a full-width level picker on Telegram; elsewhere it shows the level table. |
...
| `/aiuse` | (Fork) Show live AI subscription and API usage directly, without an AI turn (Slack: `/hermes aiuse`). |
