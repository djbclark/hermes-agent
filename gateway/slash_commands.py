...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        # Between turns, the most recent coherent per-call route wins even over
        # a cached agent: the aggregate row and the cache can both retain stale
        # billing metadata after a provider switch. Only a coherent
        # model+provider tuple may override (never combine fields from
        # different calls — #87227); the dominant main-loop route is the
        # fallback when no recent tuple exists.
        if not is_running and latest_usage:
            latest_model = _clean_str(latest_usage.get("model"))
            latest_provider = _clean_str(latest_usage.get("billing_provider"))
            if latest_model and latest_provider:
                model_name = latest_model
                provider_name = latest_provider
