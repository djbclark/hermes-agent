...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    # INVOCATION_ID / JOURNAL_STREAM are set by systemd units. Fork: PID 1 is launchd on macOS
    # and may be systemd, s6, tini or another init on Linux — do not label every PID-1 shutdown
    # as systemd; record the actual supervisor instead.
    ...
    under_systemd = bool(os.environ.get("INVOCATION_ID") or os.environ.get("JOURNAL_STREAM"))
    if under_systemd:
        supervisor = "systemd"
    elif sys.platform == "darwin" and (ppid == 1 or xpc_service_name not in (None, "", "0")):
        supervisor = "launchd"
...
def format_context_for_log(ctx: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"supervisor={ctx.get('supervisor') or ('systemd' if ctx.get('under_systemd') else 'none')} "
