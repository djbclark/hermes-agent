...
def _resolve_gateway_exit_verdict(runner, signal_initiated_shutdown: bool) -> bool:
    ...
    if signal_initiated_shutdown and not runner._restart_requested:
        if not should_restart_after_signal():
            # Fork (docs/rca-launchd-macos-crash-loop-protection-2026-08-08.md): under launchd a
            # non-zero exit after a planned SIGTERM re-arms KeepAlive into a respawn loop, so a
            # signal-initiated shutdown without a restart request must exit cleanly there.
            logger.info(
