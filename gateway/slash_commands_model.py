...
class GatewayModelCommandsMixin:
    ...
    async def _try_send_choice_picker(
        ...
        a failed send returns False (text fallback) instead of erroring. ``full_width`` (fork) lays
        each choice out on its own row where the platform has a row concept (Telegram)."""
    ...
    async def _handle_clinepass_command(self, event: MessageEvent) -> Optional[str]:
        """Handle /clinepass — switch to ClinePass at a capability level (fork).

        Bare `/clinepass` opens the interactive level picker on platforms that support one (same
        gate as `/model`, `/reasoning` and `/fast`), falling back to the text level table
        elsewhere — it does NOT silently apply the default level, so a tap is always an informed
        choice. A named level applies directly via `_apply_clinepass_level`."""
        from hermes_cli.clinepass_command import parse_clinepass_args, picker_choices, picker_title, status_text
        ...
        if not request.explicit:
            _source = await asyncio.to_thread(self._normalize_source_for_session_key, event.source)
            session_key = self._session_key_for_source(_source)
            current_model, current_effort = self._current_clinepass_selection(event, session_key)
            _persist = request.persist_global

            async def _on_clinepass_choice(_chat_id: str, value: str) -> str:
                return await self._apply_clinepass_level(event, value, persist_global=_persist)

            picker_sent = await self._try_send_choice_picker(
                event, session_key, title=picker_title(current_model, current_effort),
                choices=picker_choices(current_model, current_effort),
                on_choice_selected=_on_clinepass_choice, full_width=True,
            )
            if picker_sent:
                return None  # Picker sent — adapter handles the response
            return status_text()
        return await self._apply_clinepass_level(event, request.level, persist_global=request.persist_global)

    def _current_clinepass_selection(self, event: MessageEvent, session_key: str) -> tuple:
        """Best-effort (model, effort) for this session, for picker highlighting. Purely cosmetic —
        a resolution failure must not stop the picker from rendering."""
        current_model = str(
            ((getattr(self, "_session_model_overrides", {}) or {}).get(session_key) or {}).get("model") or "")
        current_effort = ""
        try:
            rc = self._resolve_session_reasoning_config(source=event.source, session_key=session_key, model=current_model)
            if rc is not None:
                current_effort = "none" if rc.get("enabled") is False else str(rc.get("effort") or "")
    ...
    async def _apply_clinepass_level(self, event: MessageEvent, level: str, persist_global: bool = False) -> str:
        """Apply one ClinePass level (typed or picked) and return the reply. Thin composition:
        resolve the level to a (model, effort) pair, delegate the model switch to
        `_handle_model_command` via a synthetic /model event (so session overrides, DB write-through,
        switch notes and agent eviction behave exactly like /model), then apply the effort through
        the same applier /reasoning uses. Session-scoped by default; --global persists both."""
        from gateway.run import _platform_config_key
        from hermes_cli.clinepass_command import parse_clinepass_args

        request = parse_clinepass_args(f"{level} --global" if persist_global else level)
        if request.error:
            return f"❌ {request.error}"
        flags = " --global" if request.persist_global else ""
        model_event = dataclasses.replace(event, text=f"/model {request.model} --provider cline{flags}")
        switch_reply = await self._handle_model_command(model_event)
        # Confirm the switch actually landed before touching reasoning: a failed switch stores no
        # session override, and its reply text is localized (not reliably prefix-matchable).
        _source = await asyncio.to_thread(self._normalize_source_for_session_key, event.source)
        session_key = self._session_key_for_source(_source)
        applied = (self._session_model_overrides.get(session_key) or {}).get("model") == request.model
        if not applied:
            return switch_reply
        platform_key = _platform_config_key(event.source.platform)
        effort_reply = self._apply_reasoning_selection(
            session_key, platform_key, request.effort, persist_global=request.persist_global)
        header = f"🎚️ ClinePass level `{request.level}` → `{request.model}` @ `{request.effort}`"
        return "\n".join(part for part in (header, switch_reply, effort_reply) if part)
