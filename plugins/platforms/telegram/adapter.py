...
class TelegramAdapter(BasePlatformAdapter):
    ...
    # Fork: after a gateway restart, silently continue the conversation instead of sending a
    # "session restored, what next?" prompt. Per-chat override via the ``interactive_resume`` extra.
    ...
    # Fork: "Copy to clipboard" button state (see _register_clipboard_copy): short-lived, bounded so
    # an unattended bot can't accumulate entries holding full response text indefinitely.
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Fork clipboard-copy button state: token → {text, chat_id, thread_id, created}. Holds the
        # complete unsplit response for the button on the final message of a split reply. Never
        # put into callback_data — see _register_clipboard_copy.
    ...
    async def _send_chunk_with_retries(
        ...
        thread_id: Optional[str], used_thread_fallback: bool, error_types: tuple, reply_markup=None):
        """Deliver one chunk: routing, up to 3 attempts, thread-not-found / deleted-anchor / flood handling.
        ``reply_markup`` (fork) attaches an inline keyboard — as markup, so it never counts against the chunk budget.
    ...
    async def send(
        ...
        try:
            ...
            # Fork: split legacy replies get a "copy full response" button on the FINAL chunk only, and
            # only for the final user-visible reply (metadata["notify"]) — not intermediate sends. The
            # button targets the pre-split, pre-escape `content` (never leaked via callback_data).
            clipboard_copy_keyboard = None
            if len(chunks) > 1 and (metadata or {}).get("notify"):
                clipboard_copy_keyboard = self._clipboard_copy_keyboard(
                    self._register_clipboard_copy(content, chat_id=str(chat_id), thread_id=thread_id))
            for i, chunk in enumerate(chunks):
                outcome = await self._send_chunk_with_retries(
                    ...
                    reply_markup=clipboard_copy_keyboard if i == len(chunks) - 1 else None)
    ...
    # ── Fork: "copy full response to clipboard" button ───────────────────────────────

    def _clipboard_copy_button_label(self) -> str:
        return "🍎📋 Copy to Mac Clipboard" if sys.platform == "darwin" else "📋 Copy to Hermes Clipboard"

    def _prune_clipboard_copy_state(self) -> None:
        """Drop expired entries, then trim to the max-pending bound (oldest first)."""
        now = time.time()
        for token in [t for t, e in self._clipboard_copy_state.items() if now - e["created"] > self._CLIPBOARD_COPY_TTL_SECONDS]:
            self._clipboard_copy_state.pop(token, None)
        overflow = len(self._clipboard_copy_state) - self._CLIPBOARD_COPY_MAX_PENDING
        if overflow > 0:  # dicts preserve insertion order — oldest first
            for token in list(self._clipboard_copy_state.keys())[:overflow]:
                self._clipboard_copy_state.pop(token, None)

    def _register_clipboard_copy(self, text: str, *, chat_id: str, thread_id: Optional[str]) -> str:
        """Store the full unsplit response and return a short opaque token (the token, never the text,
        goes into callback_data). Bounded + TTL'd; pruned AFTER insert so the bound counts this entry."""
        import secrets

        token = secrets.token_urlsafe(9)
        self._clipboard_copy_state[token] = {
            "text": text, "chat_id": str(chat_id), "thread_id": str(thread_id) if thread_id is not None else None,
            "created": time.time()}
        self._prune_clipboard_copy_state()
        return token

    def _clipboard_copy_keyboard(self, token: str) -> "InlineKeyboardMarkup":
        return InlineKeyboardMarkup([[InlineKeyboardButton(self._clipboard_copy_button_label(), callback_data=f"cc:{token}")]])

    async def _handle_clipboard_copy_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        """``cc:<token>`` — copy the stored full response to the Hermes host clipboard (single-use)."""
        token = data.split(":", 1)[1]
        if not await self._callback_authorized(query, cb, "⛔ You are not authorized to copy this response."):
            return
        self._prune_clipboard_copy_state()
        ...
        if entry is None:
            ...
            return
        query_chat_id, query_thread_id = cb.get("chat_id"), cb.get("thread_id")
        query_thread_str = str(query_thread_id) if query_thread_id is not None else None
        entry_thread = entry.get("thread_id")
        thread_mismatch = entry_thread is not None and query_thread_str is not None and entry_thread != query_thread_str
        if str(entry["chat_id"]) != str(query_chat_id) or thread_mismatch:
            await query.answer(text="⛔ This button isn't valid in this chat.")
            return
        # Consume before writing — a slow/failed clipboard write must not leave the token clickable.
        self._clipboard_copy_state.pop(token, None)
        text = entry["text"]
        try:
            from hermes_cli.clipboard import write_clipboard_text
            wrote = write_clipboard_text(text)
        except Exception as exc:  # never fatal — the response was already delivered
            logger.warning("[%s] clipboard copy callback raised (token=%s, chat=%s, size=%d): %s",
                           self.name, token, query_chat_id, len(text), exc)
            wrote = False
        # Log token/size/status only — never the copied text.
        if wrote:
            logger.info("[%s] clipboard copy callback succeeded (token=%s, chat=%s, size=%d)", self.name, token, query_chat_id, len(text))
            await query.answer(text="🍎📋 Copied to Mac clipboard." if sys.platform == "darwin" else "📋 Copied to Hermes clipboard.")
        else:
            logger.warning("[%s] clipboard copy callback: backend unavailable (token=%s, chat=%s, size=%d)",
    ...
    async def _send_overflow_continuation(
        ...
        thread_id: Optional[str], metadata: Optional[Dict[str, Any]], finalize: bool, reply_markup=None):
        ...
        ``reply_markup`` (fork) attaches the clipboard-copy keyboard to the final continuation."""
    ...
    async def _edit_overflow_split(
        ...
        prev_id = message_id
        thread_id = self._metadata_thread_id(metadata)
        # Fork: a real split on the final edit gets a "copy full response" button on the last
        # continuation — same contract as send()'s legacy split path; targets the pre-split `content`.
        clipboard_copy_keyboard = None
        ...
        for chunk_index, chunk in enumerate(chunks[1:], start=1):
            ...
            sent_msg = await self._send_overflow_continuation(
                ...
                reply_markup=clipboard_copy_keyboard if chunk_index == last_chunk_index else None)
            if sent_msg is None:
    ...
    async def send_choice_picker(
        ...
        metadata: Optional[Dict[str, Any]] = None, full_width: bool = False) -> SendResult:
        """Flat inline-keyboard picker (one tap → one value) for /reasoning, /fast, etc. Each choice dict:
        ``{"value": str, "label": str, "is_current": bool}``. ``full_width`` (fork) puts each button on
        its own row so long labels (e.g. /clinepass levels) get the whole width instead of two-up."""
        def build():
            buttons = []
            ...
            keyboard = InlineKeyboardMarkup([[b] for b in buttons] if full_width else self._rows_of_two(buttons))
    ...
    async def _handle_callback_query(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        for prefix, handler in (
            ...
            ("cc:", self._handle_clipboard_copy_callback),  # fork: clipboard-copy button
