...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    async def send_choice_picker(
        ...
        metadata: Optional[Dict[str, Any]] = None, full_width: bool = False,
    ) -> SendResult:
        """Flat select-menu picker (one selection → one value) for `/reasoning`, `/fast`,
        etc. Each choice: ``{"value": str, "label": str, "is_current": bool}``. ``full_width`` (fork)
        is a row-layout hint for keyboard platforms; a select menu has no row concept, so ignored."""
        def _build(_channel):
            embed = discord.Embed(
